...
# train a model with one input layer, 3 hidden layers and 1 output layer
...
dense1 = Layer_Dense(784, 512)  # Primo layer nascosto
activation1 = Activation_ReLU()
dense2 = Layer_Dense(512, 256)  # Secondo layer nascosto
activation2 = Activation_ReLU()
dense3 = Layer_Dense(256, 128)  # Layer di output
activation3 = Activation_ReLU()
dense4 = Layer_Dense(128, 64)  # Layer di output
activation4 = Activation_ReLU()
dense5 = Layer_Dense(64, 10)  # Layer di output
loss_activation = Activation_Softmax_Loss_CategoricalCrossEntropy()
optimizer = Optimizer_Adam(learning_rate=0.001, decay=5e-7)

# Addestra la rete
epochs = 450 # Puoi aumentare il numero di epoche per un'accuratezza migliore
for epoch in range(epochs):
    ...
    loss = loss_activation.forward(dense5.output, y_train)
    ...
    dense5.backward(loss_activation.dinputs)
    ...
    optimizer.update_parameters(dense5)
    optimizer.post_update_params()

path = "./"

# Salva i pesi del modello
save_model([dense1, dense2, dense3, dense4, dense5], 'model_3_450_epochs.json', path=path)
