...
# Save model function
def save_model(layers, filename, path='.'):
    # create the dictionary for the correct layer values
    model_data = {}
    # iterate to save the correct values of weights and biases
...
# Load model function
def load_model(layers, filename):
    # Open the file with the filename path
    with open(filename, 'r') as f:
        model_data = json.load(f)
    # Apply the correct weights and biases to the correct layer
    for i, layer in enumerate(layers):
        layer_data = model_data[f'layer_{i}']
